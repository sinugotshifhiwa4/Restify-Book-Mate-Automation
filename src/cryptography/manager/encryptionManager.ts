...
import StageFileManager from "../../configuration/environment/dotenv/manager/stageFileManager";
...
export class EncryptionManager {
  ...
  private async encryptAndUpdateEnvironmentVariables(
    ...
    const envFileLines = await StageFileManager.readEnvironmentFileAsLines(filePath);
    const allEnvVariables = StageFileManager.extractEnvironmentVariables(envFileLines);
    ...
    if (encryptedCount > 0) {
      await StageFileManager.writeEnvironmentFileLines(filePath, updatedLines);
    }
  ...
  private resolveVariablesToEncrypt(
    ...
    if (!envVariables?.length) {
      ...
      for (const lookupValue of envVariables) {
        ...
        const foundVariable = StageFileManager.findEnvironmentVariableByKey(
          allEnvVariables,
          trimmedLookupValue,
        );
      ...
      // Only log if there are actual not found variables
  ...
  private filterEncryptableVariables(
    ...
    if (alreadyEncrypted.length > 0) {
      logger.info(`Variables already encrypted — skipping: ${alreadyEncrypted.join(", ")}`);
    }

    if (emptyValues.length > 0) {
      logger.warn(`Variables with empty values — skipping: ${emptyValues.join(", ")}`);
    }

    // Summary of what will be processed
    if (Object.keys(variablesToEncrypt).length > 0) {
      logger.info(`Variables ready for encryption: ${Object.keys(variablesToEncrypt).join(", ")}`);
    }
    ...
    try {
      ...
      for (const [key, value] of Object.entries(variablesToEncrypt)) {
        try {
          ...
          updatedLines = StageFileManager.updateEnvironmentFileLines(
            updatedLines,
            key,
            encryptedValue,
          );
  ...
  public isAlreadyEncrypted(value: string): boolean {
    ...
    return value.startsWith(CRYPTO_CONSTANTS.FORMAT.PREFIX);
  }
  ...
  private async logEncryptionSummary(
    ...
    try {
      ...
      if (encryptedCount === 0) {
        ...
        const summary = `Encryption completed. ${encryptedCount} variables processed from '${BaseFileManager.getBaseName(filePath)}'`;
