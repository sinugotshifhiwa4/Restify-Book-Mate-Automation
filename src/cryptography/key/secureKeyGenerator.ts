...
import { CRYPTO_CONFIG } from "../types/crypto.config";
import { OUTPUT_FORMAT, CRYPTO_TYPE, CryptoGenerationOptions } from "../types/crypto.types";
...
export default class SecureKeyGenerator {
  ...
  public static generateBase64IV(length: number = this.IV_LENGTH): string {
    return this.generateSecureBytes(length, "generateBase64IV").toString(this.BASE_64);
  }
  ...
  public static generateBufferIV(length: number = this.IV_LENGTH): Buffer {
    return this.generateSecureBytes(length, "generateBufferIV");
  }
