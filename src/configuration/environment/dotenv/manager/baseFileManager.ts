...
export default class BaseFileManager {
  ...
  public static async storeEnvironmentKey(
    ...
    return this.storeKeyInFile(EnvironmentFileManager.getBaseFilePath(), keyName, value, options);
  }
  ...
  public static async verifySecretKeyExists(
    ...
    try {
      const targetFilePath = filePath || EnvironmentFileManager.getBaseFilePath();
  ...
  private static createKeyRegex(
    ...
    const pattern = this.regexPattern(keyName, captureValue);
    return new RegExp(pattern, flags);
  }

  private static regexPattern(keyName: string, captureValue = false): string {
    const escapedKeyName = this.escapeKeyNameForRegex(keyName);
    return captureValue ? `^${escapedKeyName}=(.*)$` : `^${escapedKeyName}=.*$`;
  }
